...
public class PmRequestBL
{
	...
	public static int allotedPM(PmRequestDTO pmRequest)
	{
		...
		try
		{
			result=PmRequestDL.allotedPM(pmRequest);
		}
		catch(Exception e)
		{
			System.out.print("**error**:addrto"+e.getMessage());
	...
	/*
	 * public static int UpdateStatus(PmRequestDTO pmRequest) { int result2=0; try {
	 * result2=PmRequestDL.UpdateStatus(pmRequest); } catch(Exception e) {
	 * result2=-1;
	 * System.out.println("**Error**: TempBL:UpdateStatus"+e.getMessage()); } return
	 * result2; }
	 */
}
