...
public class TempDL
{
	public static int addTempoRequest(TempDTO temp)
	{
		...
		try
		{
			...
			ps.setString(7, temp.getChassis_no());
			ps.setString(8, temp.getEngine_no());
	...
	public static ArrayList<TempDTO> getTPAllotStatus()
	{
		...
		try
		{
			...
			String sql="select * from tb_tp_alloted where status=1";
			con=DBHelper.getConnection();
			statement=con.createStatement();
			resultSet=statement.executeQuery(sql);
			while(resultSet.next())
	...
	public static int getCountTPAlloted()
	{
		...
		try
		{
			String sql="select count(*) as total from tb_tp_alloted where status=1";
			con=DBHelper.getConnection();
			statement=con.createStatement();
			resultSet=statement.executeQuery(sql);
			if(resultSet.next())
	...
	public static int allotedTP(TempDTO temp)
	{
		...
		{
			con=DBHelper.getConnection();
			query="INSERT INTO tb_tp_alloted(application_no,cus_name,chassis_no,temp_no,tp_alloted_date,expire_date,status) VALUES(?,?,?,?,?,?,?)";
			ps=con.prepareStatement(query);
			ps.setString(1,temp.getApplication_no());
			ps.setString(2, temp.getCus_name());
	...
	public static void emailSend(String application_no)
	{
		ResultSet resultSet=null;
		String query;
		Statement statement=null;
		...
		try
		{
			con=DBHelper.getConnection();
			statement=con.createStatement();
            query="Update tb_tp_alloted Set status=0 WHERE application_no='"+application_no+"'";
		...
		catch(Exception e)
		{
			...
			System.out.print("**error**:TempDLDL::emailSend()"+e.getMessage());
